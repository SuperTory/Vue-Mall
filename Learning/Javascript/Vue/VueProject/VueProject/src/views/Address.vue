            ...
            <div class="container">
                <div class="checkout-addr">
                    ...
                    <div class="addr-list-wrap">
                        <div class="addr-list">
                            <ul>
                                <li v-for="(item,index) in limitList" @click="checkIndex=index" :class="{'check':index==checkIndex}">
                                    <dl>
                                        <dt>{{item.userName}}</dt>
                                    ...
                                    <div class="addr-opration addr-del">
                                        <a @click="showConfirm(item.addressId)" href="javascript:;" class="addr-del-btn">
                                            <svg class="icon icon-del"><use xlink:href="#icon-del"></use></svg>
                                        </a>
                                    </div>
                                    <div class="addr-opration addr-set-default">
                                        <a v-if="!item.isDefault" @click="setDefault(item.addressId)" href="javascript:;" class="addr-set-default-btn"><i>设为默认</i></a>
                                    </div>
                        ...
                        <div class="shipping-addr-more">
                            <a @click="moreList" :class="{'open':listLimit>3}" class="addr-more-btn up-down-btn" href="javascript:;">
                                展开
                                <i class="i-up-down">
                ...
                <div class="order-foot-wrap">
                    <div class="prev-btn-wrap">
                        <router-link to="/cart" class="btn btn--m">上一步</router-link>
                    </div>
                    <div class="next-btn-wrap">
                        <router-link :to="{path:'/confirm',query:{checkIndex}}" class="btn btn--m btn--red">下一步</router-link>
                    </div>
                </div>
            </div>
        </div>
...
<script>
  ...
  export default {
    ...
    computed:{
      limitList(){
        return this.addressList.slice(0,this.listLimit);
      }
    },
    mounted(){
      this.getAddress();
    ...
    methods:{
      getAddress(){
        axios.get('users/address').then((response,err)=>{
      ...
      moreList(){
        if(this.listLimit===3){
          this.listLimit=this.addressList.length;
        }else{
          this.listLimit=3;
        }
      },
      ...
      showConfirm(addressId){
        this.mdShow=true;
        this.addDelete=addressId;
      },
      deleteAddress(){
        axios.post('/users/deleteAddress',{
          addressId:this.addDelete
        }).then((response,err)=>{
          if(response.data.status===0){
            this.getAddress();
            this.mdShow=false;
